...
class DataSetCreator:
    ...
    def gapTwoInTradj(self, gap=1, scenen=10, starts=0, type=("Car", "Van"), num_threshold=None) -> list:
        """
        data_root
          |-> 0000 // scene
        ...
        for scene in scene_list:
            ...
            for tid in tid_list:
                ...
                for i in range(gap, datan):
                    source_path = os.path.join(object_points_dir, points_list[i - gap])
                    target_path = os.path.join(object_points_dir, points_list[i])
                    if num_threshold is not None:
                        target = np.load(target_path)
                        if target.shape[0] < num_threshold:
                            continue
                    points = [source_path, target_path]  # source, target
